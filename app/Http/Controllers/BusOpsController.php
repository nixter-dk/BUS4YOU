...
class BusOpsController extends Controller {
    ...
    public function dashboard(Request $r) {
        ...
        return ['user'=>$r->user(),'tasks'=>$tasks,'customers'=>$customers,'employees'=>User::where('role','employee')->get(),
            ...
            'outlook'=>$r->user()->role==='admin'?['configured'=>(bool)config('services.microsoft.client_id'),'connection'=>OutlookConnection::where('user_id',$r->user()->id)->first(['email','updated_at'])]:null];
    }
