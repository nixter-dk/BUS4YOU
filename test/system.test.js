...
test('complete booking, check-in, baggage, expense and cash workflow', async () => {
  ...
  try {
    ...
    assert.equal(seatsWithPaidExtra.find(seat => seat.number === 7).passengerId, salesTicket.id);
    assert.equal(seatsWithPaidExtra.find(seat => seat.number === 7).reservationType, 'extra');
    await expectStatus(baseUrl, 409, `/api/trips/${trip.id}/passengers`, { method: 'POST', cookie: sales, body: { name: 'Forsøger optaget ekstrasæde', phone: '66666668', pickupStopId: origin.id, destinationStopId: destination.id, paymentStatus: 'cash', paymentCurrency: 'DKK', cashAmount: 100, seatNumber: 7 } });
    const salesCorrectedTicket = (await expectStatus(baseUrl, 200, `/api/trips/${trip.id}/passengers`, { method: 'PATCH', cookie: sales, body: { id: salesTicket.id, edit: true, name: 'Startsted Passager Rettet', phone: '66666667', pickupStopId: origin.id, destinationStopId: destination.id, seatNumber: 3, cashAmount: 100, paymentCurrency: 'DKK', extraSeatNumber: 7, extraSeatAmount: 75, extraSeatCurrency: 'DKK', extraSeatFree: false, correctionReason: 'Telefonnummeret var tastet forkert' } })).value;
    assert.equal(salesCorrectedTicket.cashAmount, 175);
    assert.equal(salesCorrectedTicket.extraSeatNumber, 7);
    assert.equal(salesCorrectedTicket.editHistory.at(-1).editedBy, salesManager.id);
    const driverTicket = (await expectStatus(baseUrl, 201, `/api/trips/${trip.id}/passengers`, { method: 'POST', cookie: driver, body: { name: 'Billet i bussen', phone: '67676767', pickupStopId: extraStop.id, destinationStopId: destination.id, paymentStatus: 'cash', paymentCurrency: 'EUR', cashAmount: 40, seatNumber: 4, extraSeatNumber: 8, extraSeatFree: true, extraSeatReason: 'Gratis komfortsæde' } })).value;
    ...
    assert.equal(driverTicket.extraSeatNumber, 8);
    ...
    assert.deepEqual(reports.summary.expenseTotals, { DKK: 175, EUR: 0 });
    assert.ok(reports.expenses.some(item=>item.id===salesExpense.id&&item.expenseScope==='sales_preparation'));
    assert.deepEqual(reports.tripResults[0].net, { DKK: 580, EUR: 85 });
    assert.deepEqual(reports.summary.cashAtOffice, { DKK: 705, EUR: 60 });
